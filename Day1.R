...
▪️apply : It is the base function. It takes 3 arguments : (1) data matrix; (2) row/column operation (1 for row wise operation, 2 for column wise operation); (3) function to be applied on the data.

▪️lapply : Loop over a list and evaluate a function on each element. lapply takes three arguments : (1) a list; (2) a function (or the name of a function) FUN; (3) other arguments.

▪️sapply : it will try to simplify the result of lapply if possible. It is wrapper class to lapply with difference being it returns vector or mamtrix instead of list object.

▪️tapply : It is used to apply a function over subsets of a vector.
